...
| 2 | 포트 8080 사용 여부 | `netstat -ano \| findstr :8080` | PID 표시 후 kill 여부 확인 |
| 3 | 포트 5173 사용 여부 | `netstat -ano \| findstr :5173` | PID 표시 후 kill 여부 확인 |
| 4 | H2 락 파일 | `data/vibedb.lock.db` 존재 여부 | 삭제 여부 확인 |
...
3. H2 락 파일(`data/vibedb.lock.db`) 존재 시 삭제
4. 종료 상태 보고
