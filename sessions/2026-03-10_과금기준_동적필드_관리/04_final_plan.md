# 04. Final Plan - 과금기준 동적필드 관리

## Critic 반영 사항

| # | 관점 | 이슈 | 반영 내용 |
|---|---|---|---|
| 1 | 요구사항 누락 | BillStdServiceImpl의 create/update 메서드에 `@Transactional` 미명시. 요구사항 2절 "공통 필드 + 동적 필드값 동시 저장 보장" 위반 가능 | 단계 4 상세에 create/update 메서드의 `@Transactional` 어노테이션 명시 추가 |
| 2 | 요구사항 혼란 | 변경 파일 목록에 `billStdApi.js` 수정("Config 조회 API 추가")이 있으나, 실제로는 별도 `billStdFieldConfigApi.js`를 신규 생성하여 처리함. billStdApi.js 수정 불필요 | 변경 파일 목록에서 `billStdApi.js` 제거 |
| 3 | 의존성/리스크 | update 시 delete-all + re-insert 구간에서 트랜잭션 미보장 시 부분 삭제 후 실패 가능 | 이슈 1과 동일하게 `@Transactional`로 해소 |
...
**create 메서드 수정 (`@Transactional` 필수):**
...
**update 메서드 수정 (`@Transactional` 필수):**
...
- **수정 파일**: schema.sql, data.sql, BillStd.java, BillStdRequestDto.java, BillStdResponseDto.java, BillStdServiceImpl.java, BillStdPage.vue, router/index.js (총 8개)
...
- **Critic 반영**: create/update에 @Transactional 명시, 변경 파일 목록에서 불필요한 billStdApi.js 제거
