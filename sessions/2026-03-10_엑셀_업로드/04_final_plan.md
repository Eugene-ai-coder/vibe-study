# 04. 최종 구현 계획서 — 대표가입 관리 엑셀 업로드/다운로드

## Critic 반영 사항

| # | 관점 | 발견 이슈 | 반영 내용 |
|---|---|---|---|
| 1 | 리스크 식별 누락 | `saveBulk()`가 기존 `save()`를 반복 호출하는 구조인데, `save()` 내부에서 `ResponseStatusException`을 throw한다. 벌크 중 1건이라도 실패하면 전체 트랜잭션 롤백되어 나머지 정상 건도 저장되지 않는 문제. 계획서에 이 리스크에 대한 처리 전략이 없었음. | Step 4 `saveBulk()` 구현에 **per-row try-catch** 패턴 추가. 각 건의 성공/실패를 결과 DTO에 개별 기록하고, 실패 건이 있어도 성공 건은 저장 완료되도록 변경. `@Transactional` 제거하여 건별 독립 커밋. |
| 2 | 요구사항 대비 과잉/누락 | 없음. 6개 핵심 기능, 비기능 요건, 제외 범위 모두 정확히 반영됨. |  |
| 3 | 단계 간 의존성 모순 | 없음. 백엔드→프론트엔드 순서, 각 Step 간 의존 관계 정합. |  |
| 4 | 구현 순서 논리적 타당성 | 적절함. |  |
...
- 벌크 저장은 기존 `save()` 단건 로직을 재사용하되, 건별 독립 처리 (per-row try-catch)로 부분 성공 허용
...
> **Critic 반영:** `saveBulk()` 반환 타입을 `List<SubscriptionMainResponseDto>` 에서 `List<SubscriptionMainExcelResponseDto>`로 변경. 건별 성공/실패 결과(`valid`, `errorMessage`)를 프론트엔드에 전달하기 위함.
...
**(3) saveBulk()** — **건별 독립 처리 (Critic 반영)**

```java
// @Transactional 없음 — 건별 독립 커밋
...
> **Critic 반영 핵심:** 기존 `save()`가 `ResponseStatusException`을 throw하는 구조이므로, per-row try-catch로 감싸서 부분 성공을 허용한다. `@Transactional`을 붙이지 않아 각 건이 독립 커밋된다. 단, 개별 `save()`에는 이미 `@Transactional`이 선언되어 있어 건별 트랜잭션은 보장됨.
...
> **Critic 반영:** `/bulk` 엔드포인트 반환 타입을 `List<SubscriptionMainExcelResponseDto>`로 변경하고, 상태 코드를 `201` 대신 `200`으로 변경 (부분 성공 가능하므로 201은 부적절).
...
const handleBulkSave = async () => {
  ...
  // 건별 결과를 resultMap에 반영 (valid=true -> '저장완료', valid=false -> errorMessage)
...
> **Critic 반영:** 벌크 저장 결과 메시지에 부분 실패 케이스를 반영. 서버 응답의 `valid`/`errorMessage`로 건별 결과를 `resultMap`에 표시.
...
| **벌크 저장 건별 독립 처리** (Critic 반영) | 전체 `@Transactional`로 all-or-nothing | `save()` 내부에서 `ResponseStatusException`을 throw하므로, 전체 트랜잭션 시 1건 실패가 전체 롤백 유발. 부분 성공이 사용자 경험상 더 적합 |
...
| 벌크 저장 (전건 성공) | 2건 POST `/bulk` | 2건 모두 `valid=true`, `errorMessage=저장완료` |
| 벌크 저장 (부분 실패) | 유효성 에러 건 포함 3건 POST `/bulk` | 성공 건 `valid=true`, 실패 건 `valid=false` + 에러 메시지. 성공 건은 DB 반영 완료 |
...
| 벌크 저장 (전건 성공) | 변경된 2건 체크 -> 저장 | 성공 메시지, 처리결과에 '저장완료' |
| 벌크 저장 (부분 실패) | 에러 가능성 있는 건 포함 | 성공/실패 건수 메시지, 처리결과 컬럼에 건별 결과 표시 |
...
- **핵심 설계**: 엑셀 파싱은 서버(POI), 미저장 상태는 프론트 관리, 벌크 저장은 기존 단건 로직 재사용(건별 독립 처리), DataGrid 체크박스는 `selectable` 옵트인으로 기존 페이지 무영향
- **리스크 대응**: ID 충돌->AtomicLong 카운터, apiClient 헤더->개별 호출 오버라이드, 에러 행->disabledRowIds로 선택 차단, **벌크 저장 부분 실패->per-row try-catch로 건별 독립 커밋** (Critic 반영)
