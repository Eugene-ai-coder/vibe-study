...
| `SubsBillStdResponseDto.java` | 6 | 조회 결과 DTO |
...
**2-7. SubsBillStdResponseDto (신규)**

플랫 DTO — LEFT JOIN 결과를 담는다:
    ...
    public Page<SubsBillStdResponseDto> findList(String keyword, Pageable pageable) {
        String jpql = """
            SELECT new com.example.vibestudy.SubsBillStdResponseDto(
                ...
                b.billStdId, b.stdRegStatCd, b.effStartDt, b.effEndDt
            )
            FROM Subscription s
...
**4-2. SubsBillStdListPage.vue (신규)**

- `MainLayout` 래핑
- SearchBar: 키워드 검색
- DataGrid: 페이징 목록
  - 컬럼: 가입ID, 가입명, 가입상태, 과금기준ID, 등록진행상태, 유효시작일, 유효종료일
...
- **핵심 주의사항**: (1)BillStdController `/todo`를 `/{id}` 위에 선언 (2)QnaRepository 커스텀 쿼리 Pageable에 Sort 미지정 (3)SubsBillStdServiceImpl LEFT JOIN ON 조건에 `lastEffYn='Y'` 포함 (4)Lombok 금지 — 수동 getter/setter (5)MenuDataInitializer count>0 guard 때문에 기존 환경은 DB 리셋 필요
