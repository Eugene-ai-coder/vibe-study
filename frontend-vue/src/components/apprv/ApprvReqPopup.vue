<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
    <div class="bg-white rounded-lg shadow-xl w-[720px] flex flex-col max-h-[90vh]">
      <!-- 팝업 헤더 (닫기만) -->
      <div class="flex items-center justify-between px-5 py-2 border-b border-gray-200">
        <span class="text-xs text-gray-400">결재요청서 미리보기</span>
        <button @click="$emit('close')" class="text-gray-400 hover:text-gray-600 text-lg leading-none">&times;</button>
      </div>

      <div class="overflow-auto flex-1">
        <div v-if="loading" class="text-center text-sm text-gray-400 py-16">불러오는 중...</div>
        <template v-else-if="detail">
          <!-- ═══ 결재서류 본문 ═══ -->
          <div class="px-8 py-6 space-y-5">

            <!-- ── 문서 제목 ── -->
            <div class="text-center">
              <h1 class="text-xl font-bold text-gray-800 tracking-widest">과금기준 결재요청서</h1>
              <div class="mt-1 h-[2px] bg-gray-800 mx-auto w-48"></div>
            </div>

            <!-- ── 결재선 (우측 정렬) ── -->
            <div class="flex justify-end">
              <table class="border-collapse text-xs text-center">
                <thead>
                  <tr>
                    <th class="border border-gray-400 bg-gray-100 px-4 py-1 font-semibold text-gray-600 w-[80px]">기안</th>
                    <th class="border border-gray-400 bg-gray-100 px-4 py-1 font-semibold text-gray-600 w-[80px]">승인</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td class="border border-gray-400 h-[52px] align-middle text-sm font-medium text-gray-700">
                      {{ auth.user?.userId || '-' }}
                    </td>
                    <td class="border border-gray-400 h-[52px] align-middle text-gray-400 text-[10px]">
                      (미결)
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>

            <!-- ── 문서 정보 ── -->
            <table class="w-full text-sm border-collapse border-t-2 border-gray-800">
              <colgroup>
                <col class="w-[110px]" />
                <col />
                <col class="w-[110px]" />
                <col />
              </colgroup>
              <tbody>
                <tr>
                  <th class="doc-th">문서번호</th>
                  <td class="doc-td font-mono">{{ detail.billStdReqId }}</td>
                  <th class="doc-th">기안일자</th>
                  <td class="doc-td">{{ formatDate(new Date()) }}</td>
                </tr>
                <tr>
                  <th class="doc-th">기안자</th>
                  <td class="doc-td">{{ auth.user?.userId || '-' }}</td>
                  <th class="doc-th">신청유형</th>
                  <td class="doc-td">
                    <span :class="getReqTypeBadgeClass(detail.reqTypeCd)">{{ REQ_TYPE_LABELS[detail.reqTypeCd] || detail.reqTypeCd }}</span>
                  </td>
                </tr>
              </tbody>
            </table>

            <!-- ── 신청 내역 테이블 ── -->
            <div>
              <div class="flex items-center gap-2 mb-2">
                <div class="w-1 h-4 bg-blue-600 rounded-full"></div>
                <h3 class="text-sm font-semibold text-gray-700">과금기준신청 내역</h3>
              </div>
              <table class="w-full text-sm border-collapse border-t-2 border-gray-800">
                <colgroup>
                  <col class="w-[110px]" />
                  <col />
                  <col class="w-[110px]" />
                  <col />
                </colgroup>
                <tbody>
                  <tr>
                    <th class="doc-th">가입ID</th>
                    <td class="doc-td font-mono">{{ detail.subsId }}</td>
                    <th class="doc-th">서비스코드</th>
                    <td class="doc-td">{{ getLabel('svc_cd', detail.svcCd) }}</td>
                  </tr>
                  <tr>
                    <th class="doc-th">기본상품</th>
                    <td class="doc-td">{{ detail.basicProdCd ? getLabel('basic_prod_cd', detail.basicProdCd) : '-' }}</td>
                    <th class="doc-th">최초신청일</th>
                    <td class="doc-td">{{ detail.firstReqDt }}</td>
                  </tr>
                  <tr>
                    <th class="doc-th">유효시작일</th>
                    <td class="doc-td">{{ detail.effStartDt || '-' }}</td>
                    <th class="doc-th">유효종료일</th>
                    <td class="doc-td">{{ detail.effEndDt || '-' }}</td>
                  </tr>
                  <tr>
                    <th class="doc-th">등록상태</th>
                    <td class="doc-td">{{ detail.stdRegStatCd || '-' }}</td>
                    <th class="doc-th">과금기준ID</th>
                    <td class="doc-td font-mono">{{ detail.billStdId || '-' }}</td>
                  </tr>
                  <!-- 동적 필드 -->
                  <template v-if="detail.fieldValues && Object.keys(detail.fieldValues).length > 0">
                    <tr v-for="(pair, idx) in dynamicFieldPairs" :key="'dyn-' + idx">
                      <th class="doc-th">{{ pair[0].label }}</th>
                      <td class="doc-td">{{ pair[0].value }}</td>
                      <th v-if="pair[1]" class="doc-th">{{ pair[1].label }}</th>
                      <td v-if="pair[1]" class="doc-td">{{ pair[1].value }}</td>
                      <td v-else colspan="2" class="doc-td"></td>
                    </tr>
                  </template>
                </tbody>
              </table>
            </div>

            <!-- ── 특기사항 ── -->
            <div>
              <div class="flex items-center gap-2 mb-2">
                <div class="w-1 h-4 bg-blue-600 rounded-full"></div>
                <h3 class="text-sm font-semibold text-gray-700">특기사항</h3>
              </div>
              <textarea v-model="apprvRemarks" rows="3" placeholder="결재 시 특기사항을 기재하세요 (선택)"
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-400 resize-none" />
            </div>

          </div>
        </template>
      </div>

      <!-- 하단 버튼 -->
      <div class="px-5 py-3 border-t border-gray-200 flex justify-end gap-2 bg-gray-50 rounded-b-lg">
        <button @click="$emit('close')"
          class="h-8 px-4 border border-gray-300 text-gray-600 rounded text-sm hover:bg-gray-100">취소</button>
        <button @click="handleSubmit" :disabled="submitting || !detail"
          class="h-8 px-5 bg-blue-600 text-white rounded text-sm font-medium hover:bg-blue-700 disabled:opacity-60">결재요청</button>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, computed, onMounted } from 'vue'
import { createApprvReq } from '../../api/apprvReqApi'
import { getBillStdReq } from '../../api/billStdReqApi'
import { useAuthStore } from '../../stores/auth'
import { useCommonCodeLabel } from '../../composables/useCommonCodeLabel'
import { useDynamicFieldConfigs, REQ_TYPE_LABELS, getReqTypeBadgeClass } from '../../composables/useDynamicFieldConfigs'

const props = defineProps({
  billStdReqId: { type: String, required: true },
  subsId: { type: String, required: true },
})

const emit = defineEmits(['close', 'submitted'])

const auth = useAuthStore()
const { getLabel } = useCommonCodeLabel(['svc_cd', 'basic_prod_cd'])
const { loadFieldConfigs, buildFieldPairs } = useDynamicFieldConfigs()
const detail = ref(null)
const loading = ref(true)
const submitting = ref(false)
const apprvRemarks = ref('')

const dynamicFieldPairs = computed(() => buildFieldPairs(detail.value?.fieldValues))

const formatDate = (d) => {
  const y = d.getFullYear()
  const m = String(d.getMonth() + 1).padStart(2, '0')
  const day = String(d.getDate()).padStart(2, '0')
  return `${y}-${m}-${day}`
}

onMounted(async () => {
  try {
    const data = await getBillStdReq(props.billStdReqId)
    detail.value = data
    if (data.svcCd) {
      await loadFieldConfigs(data.svcCd)
    }
  } catch {
    detail.value = null
  } finally {
    loading.value = false
  }
})

const handleSubmit = async () => {
  submitting.value = true
  try {
    await createApprvReq({
      billStdReqId: props.billStdReqId,
      subsId: props.subsId,
      apprvRemarks: apprvRemarks.value || null,
      createdBy: auth.user?.userId || 'SYSTEM',
    })
    emit('submitted')
    emit('close')
  } catch (err) {
    const status = err?.response?.status
    if (status === 400) {
      alert(err?.response?.data?.message || '결재요청에 실패했습니다.')
    } else {
      alert('결재요청에 실패했습니다.')
    }
  } finally {
    submitting.value = false
  }
}
</script>

<style scoped>
.doc-th {
  @apply bg-gray-50 border-b border-r border-gray-300 px-3 py-2 text-left text-xs font-semibold text-gray-600 whitespace-nowrap;
}
.doc-td {
  @apply border-b border-r border-gray-300 px-3 py-2 text-gray-800;
}
.doc-td:last-child,
.doc-th:last-child {
  @apply border-r-0;
}
</style>
