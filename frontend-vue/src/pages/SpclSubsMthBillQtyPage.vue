  ...
  <div>
    ...
    <div class="space-y-4 pb-24">
      ...
      <DataGrid
        ...
        storage-key="spclSubsMthBillQtyPage"
        title="특수가입별 월별과금량 목록"
      />
      ...
      <div class="space-y-4">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          ...
          <div class="grid grid-cols-3 gap-x-4 gap-y-3">
            ...
            <div>
              <label class="block text-xs text-gray-500 mb-1">사용월 <span class="text-blue-400">*</span></label>
              <input v-model="formData.useMth" :readonly="!isNew"
            ...
            <div>
              <label class="block text-xs text-gray-500 mb-1">과금기준ID <span class="text-blue-400">*</span></label>
              <input v-model="formData.billStdId" class="w-full h-8 border border-gray-300 rounded px-2 text-sm focus:outline-none focus:border-blue-400" />
            </div>
            <div>
              <label class="block text-xs text-gray-500 mb-1">PUE <span class="text-blue-400">*</span></label>
              <input v-model="formData.pue" class="w-full h-8 border border-gray-300 rounded px-2 text-sm focus:outline-none focus:border-blue-400" />
            </div>
          </div>
        </div>
      </div>
...
const EMPTY_FORM = {
  spclSubsId: '', useMth: '', subsId: '', billStdId: '', pue: '',
}
...
const selectedId = computed(() =>
  selected.value ? `${selected.value.spclSubsId}__${selected.value.useMth}` : null
)

const dataWithRowId = computed(() =>
  items.value.map(item => ({ ...item, _rowId: `${item.spclSubsId}__${item.useMth}` }))
)
...
const toRequestDto = () => ({
  ...
  useMth: formData.useMth || null,
  subsId: formData.subsId || null,
  billStdId: formData.billStdId || null,
...
const fetchList = async (pageNum = 0) => {
  const data = await getSpclSubsMthBillQtyList({ ...getSearchParams(), page: pageNum, size: pageSize })
...
const handleSaveConfirmAction = async () => {
  ...
  try {
    if (isNew.value) {
      const created = await createSpclSubsMthBillQty(toRequestDto())
      toFormData(created)
...
const executeDelete = async () => {
  ...
  try {
    await deleteSpclSubsMthBillQty(selected.value.spclSubsId, selected.value.useMth)
    selected.value = null
    Object.assign(formData, EMPTY_FORM)
    isNew.value = false
