  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 flex items-center gap-3">
      ...
      <select
        ...
        className="h-8 border border-gray-300 rounded-lg px-3 text-sm"
      >
      ...
      <input
        ...
        placeholder="검색어 입력"
        className="h-8 flex-1 border border-gray-300 rounded-lg px-3 text-sm"
        onKeyDown={(e) => e.key === 'Enter' && onSearch()}
      />
      <button
        onClick={onSearch}
        disabled={isSearching}
        className="h-8 px-6 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50"
      >
        {isSearching ? '조회 중...' : '조회'}
      </button>
    </div>
